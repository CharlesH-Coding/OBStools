...
def _next_pow2(n):
        return int(round(2**_np.ceil(_np.log2(n))))
...
def Phase(a):
        ...
        '''
        ph = 180/_np.pi*_np.arctan2(_np.imag(a),_np.real(a))
        return ph
...
def PZCOH(P,Z):
        ...
        f,spec_zp= _csd(Z.data,P.data, fs=fs,return_onesided=True,nfft=nfft)
        f,spec_zz= _csd(Z.data,Z.data, fs=fs,return_onesided=True,nfft=nfft)
        f,spec_pp= _csd(P.data,P.data, fs=fs,return_onesided=True,nfft=nfft)
